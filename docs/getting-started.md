# Getting started: your first Rust Fabric mod

This walks a new mod called `my-mod` from `cargo new` to a hello-world log line on a
real Minecraft 26.2 dev server. It assumes macOS/Linux shell syntax; on Windows the
steps are identical, only paths and library file names differ (see the platform table
below).

Prerequisites:

- Rust toolchain, `cargo` on PATH (edition 2024, so Rust 1.85+)
- JDK 25+
- This repository cloned and building (`./gradlew build` green)
...
- `crate-type = ["cdylib"]` is required — the adapter `dlopen`s your library.
- `[lib] name` (`my_mod`) is the name you will use everywhere: in the entrypoint
  `value`, and in the library file name inside the jar.
- Keep the default `panic = "unwind"`. Panics are caught at every FFI boundary and
  rethrown as Java exceptions; `panic = "abort"` would kill the JVM instead.
...
- `register_entrypoints!` generates the one well-known `fabric_rust_register` export
  ...
  empty names are compile errors.
- Entrypoint functions are plain `fn()` — no arguments, no return value (the v0.1
  adapter only implements zero-arg void entrypoint interfaces such as `ModInitializer`).
- The prelude gives you `error!`, `warn!`, `info!`, `debug!`, `trace!`. They log through
  ...
  (e.g. from a unit test) they fall back to stderr and never panic.
...
A local build targets your host only; that is fine for development. Shipping to other
platforms means cross-compiling for each target and packaging every resulting library —
this repo's release workflow ([.github/workflows/build.yml](../.github/workflows/build.yml))
does exactly that for its own jars and is a good template to crib from.
...
The `value` is `lib_name::fn_name` — cargo `[lib] name`, then the name you registered.
A bare `my_mod` would be shorthand for `my_mod::init`. Anything else (empty parts, three
or more `::` segments) is rejected at load time.

## 6. Assemble the jar

A fabric-language-rust mod jar is just `fabric.mod.json` plus your natives — no class
files needed. Layout inside the jar:
...
adapter looks nowhere else. For a multi-platform release you would add one
...
For a production-style test, put `fabric-language-rust-0.1.0.jar` and your jar into the
`mods/` folder of any Fabric Loader ≥ 0.19 / Minecraft 26.1+ / Java 25 installation.
...
`onInitialize()` calls your `init`. The extraction directory is content-addressed, so stale copies are never reused
after you rebuild.
...
- `does not bundle native library 'my_mod' for platform 'macos-arm64'` — the jar path
  is wrong (must be exactly `natives/<platform id>/<mapped file name>`), or the jar was
  built on/for a different platform.
- `the loaded library is not a fabric-rust library (missing fabric_rust_register)` —
...
- `reported ABI version N, but this build of fabric-language-rust requires ABI version 1`
  — SDK and adapter are out of sync; rebuild your crate against this repo's
  `fabric-rust`.
- `has no entrypoint named 'foo'; registered entrypoints: [init]` — the `value` in
  `fabric.mod.json` names an entrypoint you did not register; the message lists what the
  library actually registered.
- A Rust panic inside your `init` does not crash the game: it is caught, rethrown as a
  `RuntimeException` naming the entrypoint, and reported by Fabric like any other failed
  mod initializer.

## Next steps

- Raw JNI access: the SDK re-exports the `jni` crate (0.22) as `fabric_rust::jni` if
  you need to touch Java objects directly.
- Careful with threads: code on Rust-spawned threads must not call JNI `FindClass`
  (wrong classloader); the SDK's log macros are safe from any JVM-attached thread and
  fall back to stderr from unattached ones.
- Read [DESIGN.md](../DESIGN.md) for the full FFI contract, and
  [`rust/example-mod`](../rust/example-mod) + [`example/`](../example) for the
